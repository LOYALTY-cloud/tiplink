...
async function test3_apiGates() {
  ...
  // Invalid amount → 400
  {
    ...
    // Create a temp user just for gate tests
    const tempEmail = `test-wd-gates-${Date.now()}@test-wd-gates.dev`;
    const tempPwd = `WdGates${Math.random().toString(36).slice(2)}!Aa1`;
    const { data: created } = await db.auth.admin.createUser({ email: tempEmail, password: tempPwd, email_confirm: true });
    const tempUserId = created?.user?.id;

    if (!tempUserId) { skip("Could not create temp user for gate tests"); return; }

    const { data: sess } = await anonClient.auth.signInWithPassword({ email: tempEmail, password: tempPwd });
    const token = sess?.session?.access_token;

    if (!token) {
      await db.auth.admin.deleteUser(tempUserId);
      skip("Could not sign in temp user for gate tests");
      return;
    }

    const { status: s1, json: j1 } = await api({ amount: -10, payout_type: "instant" }, token);
    if (s1 === 400) pass("Negative amount → 400");
    else fail("Expected 400 for negative amount", `got ${s1}: ${JSON.stringify(j1)}`);

    const { status: s2, json: j2 } = await api({ amount: 0, payout_type: "instant" }, token);
    if (s2 === 400) pass("Zero amount → 400");
    else fail("Expected 400 for zero amount", `got ${s2}: ${JSON.stringify(j2)}`);

    // Cleanup
    await db.auth.admin.deleteUser(tempUserId);
    pass("Gate test temp user cleaned up");
  }
}
