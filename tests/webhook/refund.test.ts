        ...
        return {
          select: () => ({
            eq: (_col: string, val: any) => ({
              maybeSingle: async () => ({ data: processedRefunds[String(val)] ? { refund_id: String(val) } : null }),
            }),
          }),
        };
      }
...
async function run() {
  ...
  const intent = { id: `intent-${crypto.randomUUID()}`, creator_user_id: creator, amount: 50, receipt_id: receiptId, status: "succeeded" };
  ...
  if (ledger[0].type !== "tip_refunded" || ledger[0].amount !== -50) {
    console.error("Ledger entry incorrect", ledger[0]);
    process.exit(3);
  }

  console.log("Refund webhook test OK");
  process.exit(0);
}
