...
export async function GET(req: Request) {
  try {
    ...
    if (error) return NextResponse.json({ error: "Failed to fetch refunds." }, { status: 500 });
...
export async function POST(req: Request) {
  try {
    ...
    const { data: tip, error: tipErr } = await supabaseAdmin
      .from("tip_intents")
      .select("receipt_id, stripe_payment_intent_id, stripe_account_id, tip_amount, refunded_amount, refund_status, creator_user_id, status")
    ...
    // 6a. Direct charge: refunds must be scoped to the exact connected account
    // the PaymentIntent was created on (recorded at charge time), not a
    // possibly-changed current profile.stripe_account_id.
