...
export function alertAdmins(
  ...
  const metaRows = meta
    ? Object.entries(meta)
        ...
        .map(
          ([k, v]) =>
            `<tr><td style="color:#9ca3af;padding:4px 12px 4px 0;font-size:13px;">${esc(k)}</td><td style="color:#f9fafb;font-size:13px;">${esc(String(v))}</td></tr>`,
        )
        .join("")
    : "";
  ...
  <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;background:#0b0f1a;color:#e5e7eb;padding:40px 20px;">
    <div style="max-width:520px;margin:0 auto;background:#111827;border-radius:16px;padding:28px;border:1px solid ${color}33;">
      <div style="margin-bottom:20px;">
        <span style="display:inline-block;padding:4px 10px;border-radius:6px;background:${color}20;color:${color};font-size:12px;font-weight:700;letter-spacing:0.5px;">
          ${badge}
        </span>
      </div>
      <h2 style="margin:0 0 8px;color:#f9fafb;font-size:18px;font-weight:600;">${esc(subject)}</h2>
      <p style="margin:0 0 20px;color:#9ca3af;font-size:14px;line-height:1.6;">${esc(body)}</p>
      ${metaRows ? `<div style="background:#0f172a;border:1px solid rgba(255,255,255,0.06);border-radius:10px;padding:14px;margin-bottom:20px;"><table style="width:100%;border-spacing:0;">${metaRows}</table></div>` : ""}
