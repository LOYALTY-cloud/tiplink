...
CREATE POLICY IF NOT EXISTS "store-assets public read"
  ON storage.objects FOR SELECT
  USING (bucket_id = 'store-assets');

-- Allow authenticated users to upload/update their own objects
CREATE POLICY IF NOT EXISTS "store-assets authenticated insert"
  ON storage.objects FOR INSERT
  WITH CHECK (bucket_id = 'store-assets' AND auth.role() = 'authenticated');

CREATE POLICY IF NOT EXISTS "store-assets authenticated update"
  ON storage.objects FOR UPDATE
  USING (bucket_id = 'store-assets' AND auth.role() = 'authenticated');

CREATE POLICY IF NOT EXISTS "store-assets authenticated delete"
